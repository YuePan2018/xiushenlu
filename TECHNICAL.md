...
`log` 的用时列是从 daily 记录派生出来的结果，不作为权威累计变量保存。每次写入记录后，程序会把当天“记录”小节重新交给 LLM 判断相关任务、开始/结束/直接用时，再由代码重新计算总用时并覆盖表格；明确写出的“用时/耗时/花了”会作为直接用时累加，明确开始/结束形成的多段闭合区间会按记录自带的 `HH:MM:SS` 累加。若某个区间的开始或结束记录已经是直接用时记录，程序会跳过该区间，避免同一段工作重复计时。
