...
`log` 的用时列是从 daily 记录派生出来的结果，不作为权威累计变量保存。每次写入记录后，程序会把当天“记录”小节重新交给 LLM 判断每个任务相关的记录 ID，再由代码重新计算总用时并覆盖表格；如果相关记录中出现明确写出的时长，例如“20分钟”“用时40m”“耗时1.5h”，只使用最后一次明确时长，不累加，也不再计算首尾时间差；如果没有明确时长，则使用最后一条相关记录的 `HH:MM:SS` 减去第一条相关记录的 `HH:MM:SS`，只有一条相关记录时用时留空。

任务管理表的备注功能已删除，标准表头只接受 `| 任务 | 优先级 | 预计 | 状态 | 用时 |`。历史 daily 如果仍是旧 `备注` 表头，后续 `log` 不会自动更新任务表；需要人工或单独迁移为 `用时` 表头后再继续使用自动更新。
