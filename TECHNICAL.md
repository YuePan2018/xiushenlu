...
# 3. 追加过程记录，并让 LLM 返回受限补丁更新“任务管理”表的状态；目标/日常表会派生用时
...
| `python app/main.py log "..."` | 是 | 先追加一条过程记录，再让 LLM 返回补丁，由代码更新“任务管理”表的 `状态`；目标/日常表同时派生 `用时`，xiushenlu维护表不写预计和用时；状态支持空、`○`、`✓`、`×`，其中 `×` 表示删除、取消或不再追踪；失败时保留记录。 |
...
`plan` 生成任务管理表时，`任务` 列会由程序尽量匹配并回填今日待办里的任务正文，避免把待办扩写成计划说明。任务管理固定拆成 `【目标】`、`【日常】`、`【xiushenlu维护】` 三张表：今日待办 `【日常】` 分组进入日常表；修身炉 / xiushenlu 项目的修 bug、修复、优化、维护类任务进入维护表；其他任务进入目标表。目标和日常表头是 `| 任务 | 优先级 | 预计 | 状态 | 用时 |`，维护表头是 `| 任务 | 优先级 | 状态 |`。

`plan --add` 要求模型返回严格 JSON，并逐字保留新增任务；程序会更新 `today_tasks.md`、替换 daily 里已有的“今日待办”原文，并把新增任务正文追加到对应任务管理表。目标/日常新增行保留预计、状态和用时为空；维护新增行只保留任务、优先级和状态。解析失败时不会写入 `today_tasks.md` 或 daily。

`log` 的用时列是从 daily 记录派生出来的结果，不作为权威累计变量保存。每次写入记录后，程序会把当天“记录”小节重新交给 LLM 判断每个任务相关的记录 ID，再由代码重新计算目标/日常表总用时并覆盖表格；如果相关记录中出现明确写出的时长，例如“20分钟”“用时40m”“耗时1.5h”，只使用最后一次明确时长，不累加，也不再计算首尾时间差；如果没有明确时长，只有最后一条相关记录内容包含 `&` 时，才使用最后一条相关记录的 `HH:MM:SS` 减去第一条相关记录的 `HH:MM:SS`，否则用时留空。xiushenlu维护表没有用时列，`log` 只更新状态。

任务管理表的备注功能已删除，标准表头只接受目标/日常五列表或 xiushenlu维护三列表。历史 daily 如果仍是旧 `备注` 表头，后续 `log` 不会自动更新任务表；需要人工或单独迁移后再继续使用自动更新。
...
| `app/pipelines/daily_plan.py` | 今日计划 pipeline；LLM 生成任务管理草稿，程序回填 `任务` 列为今日待办任务正文，并拆成目标、日常、xiushenlu维护三张表。 |
| `app/pipelines/log_schedule_update.py` | 写入记录后的任务管理表更新 pipeline；LLM 返回状态和计时线索，代码更新状态，并只给目标/日常表重算 `用时`。 |
| `app/pipelines/plan_update.py` | 日内计划局部更新 pipeline；更新待办原文并把新增任务追加到对应任务管理分表，解析失败时停止写入。 |
...
如果测试、验证或检查命令超时、被用户中断或疑似留下进程，先按 `AGENTS.md` 记录失败并检查进程；定位出明确原因前，不重复运行同类命令。需要防卡住时，用会主动退出的测试，或用 PowerShell 启动受控子进程并调用 `$p.WaitForExit(<milliseconds>)` 做进程级超时，超时后清理该 PID。
