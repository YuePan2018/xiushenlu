...
- next_today_tasks：新的完整 today_tasks.md 内容。只能由“今日待办”里的未完成计划任务和“明日计划.md”的显式内容组成；保持中文分组、列表/编号习惯；不要输出任何 Markdown 标题行，禁止输出任何以 `#` 开头的标题行；不要生成口号、总结句或装饰性标题；不要判断优先级；根据今日待办与今日记录判断今天未完成任务，优先按原小标题归类；再叠加“明日计划.md”的内容；去掉明显重复项。
