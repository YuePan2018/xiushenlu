...
class LogScheduleUpdateTests(unittest.TestCase):
    ...
    def test_update_schedule_handles_grouped_tables_and_skips_maintenance_table(self) -> None:
        with _temporary_directory() as temp_dir:
            config = _test_config(Path(temp_dir))
            daily_file = _write_daily(
                config,
            ...
            reply = json.dumps(
                {"updates": [{"row_index": 2, "status": "completed", "evidence": "喝水完成"}]},
                ensure_ascii=False,
            )
    ...
    def test_update_schedule_changes_maintenance_status_without_duration(self) -> None:
        with _temporary_directory() as temp_dir:
            ...
            daily_text = daily_file.read_text(encoding="utf-8")
            self.assertIn("| 修复计划表 | P1 | ✓ |", daily_text)
            self.assertNotIn("| 修复计划表 | P1 |  | ✓ | 20m |", daily_text)

    def test_log_adds_new_maintenance_task_with_status(self) -> None:
        with _temporary_directory() as temp_dir:
            config = _test_config(Path(temp_dir))
            daily_file = _write_daily(
                config,
                ...
                "|---|---|---|\n\n"
                "## 记录\n\n"
                "- 10:00:00 修复 log_schedule_update 新维护任务追加完成。\n",
            )
            reply = json.dumps(
                {
                    "updates": [],
                    "maintenance_additions": [
                        {
                            "task": "修复 log_schedule_update 新维护任务追加",
                            "priority": "P1",
                            "status": "completed",
                            "evidence": "修复 log_schedule_update 新维护任务追加完成",
                        }
                    ],
                },
                ensure_ascii=False,
            ...
            result = update_schedule_from_log(
                FakeProvider(reply),
                "修复 log_schedule_update 新维护任务追加完成。",
                config=config,
                target_date=date(2026, 5, 10),
                logger=FakeLogger(),  # type: ignore[arg-type]
            )
