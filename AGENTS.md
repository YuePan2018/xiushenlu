...
- 测试、验证或检查命令一旦出现超时、被用户中断、工具超时未生效或留下残留进程，必须先记录失败、检查并清理本次残留进程，再分析最近改动中的循环/阻塞/进程等待原因；在找到并修复明确原因前，禁止继续运行同类测试命令。需要再次验证时，优先使用受控子进程方式，显式记录 PID，并用进程级超时清理，而不是依赖工具 `timeout_ms`。
