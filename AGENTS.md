...
- `docs/规划/2026-05-15_修身炉规划.md`：最新阶段规划和当前能力边界。
...
1. 知识库建立：把资料整理为主题、来源、时间、核心观点、行动项、项目启发和复用经验。
2. 自动维护和升级知识：生成周复盘、主题索引、知识缺口、过期提醒、workflow 草稿、skill 草稿和工具改进建议。
...
- 如果失败命令是在运行 `tests/` 目录下的测试（包括 `pytest tests/...`、`unittest tests.xxx`、`unittest tests/xxx.py` 等），额外写一行固定标记：`标记：TESTS_DIR_FAIL`，方便后续统计测试运行失败。
